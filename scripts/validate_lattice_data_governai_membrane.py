#!/usr/bin/env python3
"""Validate Lattice Data/GovernAI New Hope membrane fixtures."""
...
FIXTURE = ROOT / "fixtures" / "lattice" / "lattice-data-governai-membrane" / "membrane-fixture.v0.1.json"

REQUIRED_ASSET_KINDS = {"data-product", "evaluation-bundle", "publication-artifact"}
...
def validate_asset(asset: dict[str, Any]) -> str:
    ...
    if kind != "data-product":
        require("urn:srcos:data-product:community_truth_demo" in lineage, f"{kind} must preserve DataProduct lineage")
...
def main() -> int:
    ...
    try:
        ...
        require(data.get("kind") == "LatticeDataGovernAIMembraneFixture", "kind mismatch")
        metadata = data.get("metadata")
        require(isinstance(metadata, dict), "metadata must be object")
        require(metadata.get("name") == "lattice-data-governai-membrane", "metadata.name mismatch")
        ...
        for key in ["sherlockFixtureRef", "policyFixtureRef", "topologyFixtureRef"]:
